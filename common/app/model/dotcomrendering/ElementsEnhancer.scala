...
object ElementsEnhancer {
  ...
  def enhanceElement(element: JsValue): JsValue = {
    ...
    // If elements have further nesting, continue to enhance
    elementType match {
  ...
  def enhanceListBlockElement(element: JsObject): JsObject = {
    val itemsList = element.value("items").as[JsArray]
    val itemsListWithIds = itemsList.value.map { item =>
      val enhancedItem = enhanceElements(item.as[JsArray])
      item.as[JsObject] ++ Json.obj("elements" -> enhancedItem)
    }
    element ++ Json.obj("items" -> itemsListWithIds)
  }
